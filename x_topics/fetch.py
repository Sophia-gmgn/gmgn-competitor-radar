...
硬性要求：
1. 【只算加密 / meme 交易语境下、确实关于「{label}」的讨论】——忽略同名的无关内容（如同名的音乐人、物理名词、网络俚语、其它行业公司等）。
2. 【排除拉新导流帖】：主体是推荐码 / 邀请链接 / referral / "用我的码上车" / "点我链接注册领奖励" 这类拉新帖，不要当作话题，也不要放进 examples；正常讨论里顺带提到产品的，保留。
3. 【不编造】：这段时间没有真实讨论就直接返回空数组 []，不要用训练知识补。
4. 最终必须是能被 JSON.parse 直接解析的合法 JSON。"""
...
def main():
    ...
    print(f"\n=== 汇总 ===")
    print(f"本次话题 {len(items)} 个；底稿 {before} → {len(store)}（新增 {added}）")
    if failed:
